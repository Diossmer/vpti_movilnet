...
class InventariosController extends Controller
{
    public function index()
    {
        try {
            if(Auth::check()){
                $inventarios = Inventarios::with('estatus','productos')->get();
    ...
    public function exportar(string $id=null){
        try {
            if(is_numeric($id)){
                $data = new ExportMultiSheet(Inventarios::with('estatus','productos')->where('id','=',$id)->get()->makeHidden(['id']));
                return ($data)->download('*.xlsx');
            }
            $data = new ExportMultiSheet(Inventarios::with('estatus','productos')->get()->makeHidden(['id']));
            return ($data)->download('*.xlsx');
        } catch (\Exception $e) {
