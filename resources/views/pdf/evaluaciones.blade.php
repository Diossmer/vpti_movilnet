@if($subtitle === 'informes' || $subtitle === 'informe')
  ...
  <body>
    ...
    <main class="report-container">
      ...
      <p class="text">A continuación, se detalla la información completa del inventario de evaluaciones con fines de seguimiento y auditoría interna. 
      ...
      @if($evaluaciones->isEmpty())
        <p>No se encontraron registros de inventario de evaluaciones.</p>
      @else
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            @foreach($evaluaciones as $evaluacion)
              <tr>
                ...
                <td>{{ $evaluacion->escala ?? '' }}</td>
                <td>{{ $evaluacion->compatibilidad ?? '' }}</td>
                <td>{{ $evaluacion->reemplazo ?? '' }}</td>
                <td>{{ $evaluacion->mantenimiento ?? '' }}</td>
                <td>{{ $evaluacion->estatus->nombre ?? '' }}</td>
              </tr>
              <tr class="description-row">
                <td colspan="6" class="description-cell">
                  <strong>Descripción:</strong> {{ $evaluacion->descripcion->observacion ?? '' }}
                </td>
              </tr>
            @endforeach
          </tbody>
